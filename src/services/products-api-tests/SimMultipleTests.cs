...
namespace products_api_tests
{
    public class SimMultipleTests
    {
        ...
        public async void FullWorkflow()
        {
            ...
            var controllerName = "SimMultiples";
            ...
            var createDto = new SimMultipleCreateDto { Name = newName, Position = newPosition };
            // Send add request
            var responseAdd = await client.PostAsJsonAsync(addUrl, createDto);
            var createdDto = await responseAdd.Content.ReadFromJsonAsync<ServiceResponse<SimMultipleDto>>();
            ...
            var updateDto = new SimMultipleUpdateDto { Name = updatedName, Position = updatedPosition };
            var responseUpdate = await client.PutAsJsonAsync(updateUrl, updateDto);
            var updatedDto = await responseUpdate.Content.ReadFromJsonAsync<ServiceResponse<SimMultipleDto>>();
