...
{
    public class IpCertificateTests
    {
        ...
        public async void FullWorkflow()
        {
            ...
            var controllerName = "IpCertificates";
            ...
            var createDto = new IpCertificateCreateDto { Name = newName, Position = newPosition };
            // Send add request
            var responseAdd = await client.PostAsJsonAsync(addUrl, createDto);
            var createdDto = await responseAdd.Content.ReadFromJsonAsync<ServiceResponse<IpCertificateDto>>();
            ...
            var updateDto = new IpCertificateUpdateDto { Name = updatedName, Position = updatedPosition };
            var responseUpdate = await client.PutAsJsonAsync(updateUrl, updateDto);
            var updatedDto = await responseUpdate.Content.ReadFromJsonAsync<ServiceResponse<IpCertificateDto>>();
