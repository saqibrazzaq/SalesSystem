...
{
    public class DisplayTechnologyTests
    {
        ...
        public async void FullWorkflow()
        {
            ...
            var controllerName = "DisplayTechnologies";
            ...
            var createDto = new DisplayTechnologyCreateDto { Name = newName, Position = newPosition };
            // Send add request
            var responseAdd = await client.PostAsJsonAsync(addUrl, createDto);
            var createdDto = await responseAdd.Content.ReadFromJsonAsync<ServiceResponse<DisplayTechnologyDto>>();
            ...
            var updateDto = new DisplayTechnologyUpdateDto { Name = updatedName, Position = updatedPosition };
            var responseUpdate = await client.PutAsJsonAsync(updateUrl, updateDto);
            var updatedDto = await responseUpdate.Content.ReadFromJsonAsync<ServiceResponse<DisplayTechnologyDto>>();
