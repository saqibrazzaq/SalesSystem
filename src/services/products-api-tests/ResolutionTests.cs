...
namespace products_api_tests
{
    public class ResolutionTests
    {
        ...
        public async void FullWorkflow()
        {
            ...
            var controllerName = "Resolutions";
            ...
            var createDto = new ResolutionCreateDto { Name = newName, Position = newPosition };
            // Send add request
            var responseAdd = await client.PostAsJsonAsync(addUrl, createDto);
            var createdDto = await responseAdd.Content.ReadFromJsonAsync<ServiceResponse<ResolutionDto>>();
            ...
            var updateDto = new ResolutionUpdateDto { Name = updatedName, Position = updatedPosition };
            var responseUpdate = await client.PutAsJsonAsync(updateUrl, updateDto);
            var updatedDto = await responseUpdate.Content.ReadFromJsonAsync<ServiceResponse<ResolutionDto>>();
