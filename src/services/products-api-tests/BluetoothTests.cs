...
namespace products_api_tests
{
    public class BluetoothTests
    {
        ...
        public async void FullWorkflow()
        {
            ...
            var createDto = new BluetoothCreateDto { Name = newName, Position = newPosition };
            // Send add request
            var responseAdd = await client.PostAsJsonAsync(addUrl, createDto);
            var createdDto = await responseAdd.Content.ReadFromJsonAsync<ServiceResponse<BluetoothDto>>();
            ...
            var updateDto = new BluetoothUpdateDto { Name = updatedName, Position = updatedPosition };
            var responseUpdate = await client.PutAsJsonAsync(updateUrl, updateDto);
            var updatedDto = await responseUpdate.Content.ReadFromJsonAsync<ServiceResponse<BluetoothDto>>();
