...
{
    public class FrameMaterialTests
    {
        ...
        public async void FullWorkflow()
        {
            ...
            var controllerName = "FrameMaterials";
            ...
            var createDto = new FrameMaterialCreateDto { Name = newName, Position = newPosition };
            // Send add request
            var responseAdd = await client.PostAsJsonAsync(addUrl, createDto);
            var createdDto = await responseAdd.Content.ReadFromJsonAsync<ServiceResponse<FrameMaterialDto>>();
            ...
            var updateDto = new FrameMaterialUpdateDto { Name = updatedName, Position = updatedPosition };
            var responseUpdate = await client.PutAsJsonAsync(updateUrl, updateDto);
            var updatedDto = await responseUpdate.Content.ReadFromJsonAsync<ServiceResponse<FrameMaterialDto>>();
