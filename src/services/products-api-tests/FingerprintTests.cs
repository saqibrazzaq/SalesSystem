...
namespace products_api_tests
{
    public class FingerprintTests
    {
        ...
        public async void FullWorkflow()
        {
            ...
            var controllerName = "Fingerprints";
            ...
            var createDto = new FingerprintCreateDto { Name = newName, Position = newPosition };
            // Send add request
            var responseAdd = await client.PostAsJsonAsync(addUrl, createDto);
            var createdDto = await responseAdd.Content.ReadFromJsonAsync<ServiceResponse<FingerprintDto>>();
            ...
            var updateDto = new FingerprintUpdateDto { Name = updatedName, Position = updatedPosition };
            var responseUpdate = await client.PutAsJsonAsync(updateUrl, updateDto);
            var updatedDto = await responseUpdate.Content.ReadFromJsonAsync<ServiceResponse<FingerprintDto>>();
