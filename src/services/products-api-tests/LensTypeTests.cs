...
namespace products_api_tests
{
    public class LensTypeTests
    {
        ...
        public async void FullWorkflow()
        {
            ...
            var createDto = new LensTypeCreateDto { Name = newName, Position = newPosition };
            // Send add request
            var responseAdd = await client.PostAsJsonAsync(addUrl, createDto);
            var createdDto = await responseAdd.Content.ReadFromJsonAsync<ServiceResponse<LensTypeDto>>();
            ...
            var updateDto = new LensTypeUpdateDto { Name = updatedName, Position = updatedPosition };
            var responseUpdate = await client.PutAsJsonAsync(updateUrl, updateDto);
            var updatedDto = await responseUpdate.Content.ReadFromJsonAsync<ServiceResponse<LensTypeDto>>();
